...
export const distance = (graph, route) => {
  const acc0 = { tailId: route[0], dist: 0 }; // first node in route is tail of first edge to measure, distance measured so far is 0
  try {
    return route.slice(1)
      .reduce((acc, headId) =>({
        tailId: headId, // make current head node tail node in next iteration
        dist: acc.dist + edgeDistance(graph[acc.tailId], headId) // increment by distance from tail to head (or throw)
      }), acc0).dist; // return only distance portion of accumulator
  } catch (e) {
